...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    ...
    texto = FONTE_PONTOS.render(f"Pontuação: {pontos}", 1, (255, 255, 255)) #o 'f' antes da string informa que uma variável vai ser passada dentro da própria string, no caso, pontos
...
def main():
    ...
    while rodando:  # o jogo é um loop infinito
        ...
        else: #se ouver colisão = game over
            # Tela de Game Over
            fonte_game_over = pygame.font.SysFont('arial', 60)
            texto_game_over = fonte_game_over.render('Game Over', True, (255, 0, 0))
            tela.blit(texto_game_over, (TELA_LARGURA // 2 - texto_game_over.get_width() // 2, ALTURA_TELA // 2 - texto_game_over.get_height() // 2))
            pygame.display.update()
